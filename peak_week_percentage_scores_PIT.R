  ...
  for (model in models_to_score){
    for (test_week in c(paste0(year,seq(41,52)),paste0(year+1,"0",1:9),paste0(year+1,seq(10,20)))){
      for (region in unique(data$region)){
        ...
        result_df <- rbind(result_df,c(model,year,test_week,region,true_peak_week_percentage,prob))
...
saveRDS(result_df,"./result_objects_PIT/peak_week_percentage_result_df")
