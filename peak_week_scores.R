  ...
  for (model in models_to_score){
    ...
    for (test_week in test_weeks){
       for (region in unique(data$region)){
        ...
        #print (prob)
        result_df <- rbind(result_df,c(model,year,test_week,region,true_onset_week,log(single_bin_prob)))
...
saveRDS(result_df,"./result_objects_single_bin/peak_week_result_df")
